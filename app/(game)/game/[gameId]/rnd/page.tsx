...
export default function RDPage({ params }: PageProps) {
  ...
  }), [rdInvestment, pendingProducts, pendingTechUpgrades, pendingPatentFilings, pendingLicenses, pendingChallenges, productImprovements]);
          ...
          <Card className="bg-slate-800/80 border-slate-700/50 backdrop-blur-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                ...
                <div className="p-3 bg-slate-700/50 rounded-lg">
                  <div className="text-slate-400 text-sm">Improvement Effectiveness</div>
                  <div className="text-green-400 font-medium">{rdInvestment >= 15_000_000 ? "1.5x" : rdInvestment >= 10_000_000 ? "1.2x" : rdInvestment >= 5_000_000 ? "1.0x" : "0.7x"}</div>
                  <div className="text-slate-500 text-xs">Multiplier on Q/F improvements</div>
                </div>
                <div className="p-3 bg-slate-700/50 rounded-lg">
                  <div className="text-slate-400 text-sm">Decay Protection</div>
                  <div className="text-green-400 font-medium">{Math.min(70, Math.round((rdInvestment / 15_000_000) * 70))}%</div>
                  <div className="text-slate-500 text-xs">Reduces product aging</div>
                </div>
              </div>
            </CardContent>
          </Card>
                    ...
                    <div className="p-4 bg-purple-900/30 border border-purple-700 rounded-lg">
                      ...
                      <div className="grid md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <div className="text-white font-medium">
                            ${Math.round(({"Budget":60,"General":150,"Enthusiast":350,"Professional":600,"Active Lifestyle":250}[selectedSegment || "General"] || 150) + qualityTarget * 2 + featuresTarget * 1.5)}
                          </div>
                        </div>
                        <div>
                          <span className="text-slate-400">Est. Margin</span>
                          <div className={`font-medium ${priceTarget > (({"Budget":60,"General":150,"Enthusiast":350,"Professional":600,"Active Lifestyle":250}[selectedSegment || "General"] || 150) + qualityTarget * 2 + featuresTarget * 1.5) ? "text-green-400" : "text-red-400"}`}>
                            {Math.round((1 - (({"Budget":60,"General":150,"Enthusiast":350,"Professional":600,"Active Lifestyle":250}[selectedSegment || "General"] || 150) + qualityTarget * 2 + featuresTarget * 1.5) / priceTarget) * 100)}%
                          </div>
                        </div>
                      </div>
                      <Button
                        ...
                        <div className="flex items-start justify-between">
                          ...
                          {isOwned || pendingTechUpgrades.includes(tech.id) ? (
                            ...
                            <Badge variant="outline" className="border-slate-500">
                              Requires: R&D Level {tech.requires}
                            </Badge>
                          ) : (
                            <div className="text-right">
