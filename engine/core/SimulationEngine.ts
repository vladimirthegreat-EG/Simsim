...
export class SimulationEngine {
  ...
  static processRound(input: SimulationInput): SimulationOutput {
    ...
    // Clone input states first to avoid mutating the caller's objects
    const teamsWithClonedState = input.teams.map(team => ({
      ...team,
      state: cloneTeamState(team.state),
    }));
    for (const team of teamsWithClonedState) {
    ...
    for (const team of teamsWithClonedState) {
      ...
      // State already cloned in Step 0.5 — use directly
      let currentState = team.state;
      ...
      // Continuous Improvement: +1% efficiency per round for factories with upgrade (cap 1.0)
      for (const factory of currentState.factories) {
        if (factory.continuousImprovementEnabled && roundNumber > 1) {
          factory.continuousImprovementBonus = Math.min(
            0.30, // Cap at +30% total bonus (so 0.7 base → 1.0 max)
            (factory.continuousImprovementBonus ?? 0) + 0.01
          );
        }
      }

      // FactoryModule.calculateStaffingPenalty() — apply as workforce efficiency drag
      ...
      // FIX #9: Sync average worker accuracy to factories for defect rate calculation
      if (currentState.employees.length > 0) {
        const workerEmployees = currentState.employees.filter(e => e.role === "worker");
        const avgAccuracy = workerEmployees.length > 0
          ? workerEmployees.reduce((sum, e) => sum + (e.stats as any).accuracy, 0) / workerEmployees.length
          : 75;
        for (const factory of currentState.factories) {
          (factory as any).avgWorkerAccuracy = avgAccuracy;
        }
      }
    ...
    for (const team of processedTeams) {
      ...
      // FinanceModule.calculateInvestorSentiment() — replaces hardcoded 50
      ...
      for (const factory of team.state.factories) {
        ...
        // Max capacity = MIN(worker capacity, machine capacity) × efficiency
        // Capsim/BSG pattern: you need both workers AND machines
        const workerCapacity = factory.workers * CONSTANTS.BASE_WORKER_OUTPUT;
        const machineCapacity = (factory.productionLines ?? []).reduce(
          (sum, line) => sum + (line.capacity ?? 0), 0
        );
        const rawCapacity = machineCapacity > 0
          ? Math.min(workerCapacity, machineCapacity)
          : workerCapacity; // fallback if no production lines
        const maxCapacity = rawCapacity * factory.efficiency;
    ...
    // FinanceModule.calculateEPSRanking() — compute EPS percentile for each team
  ...
  static createInitialTeamState(
    ...
    // Medium factory = 6 lines max — start with 2 active (General + Budget), 4 spare for expansion
    const numSegments = presetConfig?.startingSegments ?? 2;
    ...
    const baseState: TeamState = {
      ...
      // Starter machines — 1 Assembly Line per active production line
      machineryStates: {
        [defaultFactory.id]: {
          machines: starterMachines as any[],
          totalCapacity: starterMachines.reduce((s, m) => s + m.capacityUnits, 0),
          totalMaintenanceCost: starterMachines.reduce((s, m) => s + m.maintenanceCostPerRound, 0),
          totalOperatingCost: starterMachines.reduce((s, m) => s + m.operatingCostPerRound, 0),
          averageHealth: 100,
        },
      },
  ...
  static createInitialMarketState(): MarketState {
    return {
      ...
      // Demand growth tuned so 5 teams reach ~80% market coverage by round 32
      // Mass markets grow slower (mature), niche segments grow faster (emerging)
      demandBySegment: {
        Budget: { totalDemand: 700000, priceRange: { min: 100, max: 300 }, growthRate: 0.02 },
        General: { totalDemand: 450000, priceRange: { min: 300, max: 600 }, growthRate: 0.02 },
        Enthusiast: { totalDemand: 150000, priceRange: { min: 600, max: 1000 }, growthRate: 0.03 },
        Professional: { totalDemand: 80000, priceRange: { min: 1000, max: 1500 }, growthRate: 0.01 },
        "Active Lifestyle": { totalDemand: 200000, priceRange: { min: 400, max: 800 }, growthRate: 0.03 },
      },
      marketPressures: {
