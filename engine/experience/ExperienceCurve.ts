...
export class ExperienceCurveEngine {
  ...
  /**
   * Simplified Wright's Law cost multiplier (stateless utility).
   * Use this when you don't need the full segment-based engine (e.g. Monte Carlo demos).
   *
   * @param cumulativeUnits Total units produced so far
   * @param learningRate Learning rate (default 0.85, lower = faster learning)
   * @param floor Minimum cost multiplier (default 0.65)
   */
  static wrightMultiplier(
    cumulativeUnits: number,
    learningRate: number = BASE_LEARNING_RATE,
    floor: number = MIN_COST_MULTIPLIER
  ): number {
    if (cumulativeUnits <= INITIAL_PRODUCTION) return 1.0;
    const mult = Math.pow(learningRate, Math.log2(cumulativeUnits / INITIAL_PRODUCTION));
