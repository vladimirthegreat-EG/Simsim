...
describe("R&D Module — Comprehensive Stress Tests", () => {
  ...
  describe("Part A — R&D Stress Suite", () => {
    ...
    describe("Category C — Property Tests", () => {
      ...
      it("material costs per segment are correct", () => {
        expect(CONSTANTS.RAW_MATERIAL_COST_PER_UNIT.Budget).toBe(90);
        expect(CONSTANTS.RAW_MATERIAL_COST_PER_UNIT.General).toBe(200);
        expect(CONSTANTS.RAW_MATERIAL_COST_PER_UNIT.Enthusiast).toBe(320);
        expect(CONSTANTS.RAW_MATERIAL_COST_PER_UNIT.Professional).toBe(550);
        expect(CONSTANTS.RAW_MATERIAL_COST_PER_UNIT["Active Lifestyle"]).toBe(250);
      });
    });
  ...
  describe("Part B — §1.10 RDModule Deep Stress Suite", () => {
    ...
    // ─────────────────────────────────────────────
    describe("2. Patent Generation — now handled by PatentEngine only", () => {
      it("rdProgress=199 with no budget — patents unchanged (no auto-generation)", () => {
        const state = createMinimalTeamState();
        ...
        // RDModule no longer auto-generates patents — managed by PatentEngine
        expect(newState.patents).toBe(0);
      });

      it("rdProgress=200 — patents unchanged (no auto-generation from RDModule)", () => {
        const state = createMinimalTeamState();
        ...
        // Patents no longer auto-generated by RDModule
        expect(newState.patents).toBe(0);
      });

      it("rdProgress=400 — patents unchanged (no auto-generation from RDModule)", () => {
        const state = createMinimalTeamState();
      ...
      it("crossing old patent boundary mid-round via budget — patents unchanged", () => {
        const state = createMinimalTeamState();
    ...
    describe("5. Auto-Launch Ready Products", () => {
      ...
      it("in_development product transitions to launched when roundsRemaining hits 0", () => {
        ...
        expect(product).toBeDefined();
        expect(product!.developmentStatus).toBe("launched");
        // POST-FIX: Product aging (F2) + material quality blend (2C) affect final values
        expect(product!.quality).toBeGreaterThanOrEqual(65);
        expect(product!.quality).toBeLessThanOrEqual(80);
        expect(product!.features).toBeGreaterThanOrEqual(55);
        expect(product!.features).toBeLessThanOrEqual(65);
      });
    });
