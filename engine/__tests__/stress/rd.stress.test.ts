...
describe("R&D Stress — Category B: Edge Scenarios", () => {
  ...
  it("$30M R&D for 8 rounds — fast tech unlock chain", () => {
    ...
    for (const state of result.finalStates) {
      ...
      // Patents no longer auto-generated by RDModule (now handled by PatentEngine only).
      // $30M/round = 300 budget pts/round + engineer contribution.
      // Over 8 rounds, rdProgress accumulates without patent consumption.
      expect(state.rdProgress).toBeGreaterThanOrEqual(0);
      // 8 rounds × ~300 pts/round = ~2400+ accumulated rdProgress
      expect(state.rdProgress).toBeGreaterThan(1000);
  ...
  it("tech prerequisite chain integrity — cannot skip tiers", () => {
    ...
    // rdProgress should still be non-negative (no longer consumed by patent generation)
  ...
  it("rdProgress accumulation formula verification — budget + engineers", () => {
    ...
    // Patents no longer auto-generated by RDModule — no consumption to account for
    expect(actualGain).toBeCloseTo(expectedGain, 0);
  ...
  it("patent generation — no longer auto-generated by RDModule (handled by PatentEngine)", () => {
    const state = createTeamState("baseline-balanced");
    state.rdProgress = 0;
    state.patents = 0;

    // Give enough budget to generate well past the old 200-point patent threshold
    const { newState: state1 } = processRD(state, { rdBudget: 25_000_000 }, 100, 1);

    // Patents no longer auto-generated by RDModule — managed by PatentEngine
    const patents1 = typeof state1.patents === "number" ? state1.patents : 0;
    expect(patents1).toBe(0);

    // Continue accumulating — patents should still be 0 from RDModule
    ...
    expect(patents2).toBe(0);

    // rdProgress should still accumulate
    expect(state2.rdProgress).toBeGreaterThan(state1.rdProgress);
