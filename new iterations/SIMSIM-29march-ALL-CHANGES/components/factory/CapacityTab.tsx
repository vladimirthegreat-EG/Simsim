...
export function CapacityTab({
  ...
  const [selectedLineId, setSelectedLineId] = useState<string | null>(
    selectedFactory.productionLines?.[0]?.id ?? null
  );
  ...
  return (
    <motion.div
      ...
      className="space-y-6"
    >
      {/* ====== Factory Overview — Lines + Machines Per Line ====== */}
      <Card className={`border ${factoryFull ? "bg-orange-900/20 border-orange-600/50" : "bg-slate-800/80 border-slate-700/50"} backdrop-blur-sm`}>
        <CardContent className="py-4 px-5 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Factory className="w-5 h-5 text-orange-400" />
              <div>
                <p className="text-white font-semibold">{factoryTier} Factory — {numLines} Production Line{numLines !== 1 ? "s" : ""}</p>
                <p className="text-slate-400 text-sm">{usedSlots} machines total (max {MAX_MACHINES_PER_LINE} per line)</p>
              </div>
            </div>
            {factoryFull && (
              <Badge className="bg-orange-500/20 text-orange-400 text-xs">
                <AlertTriangle className="w-3 h-3 mr-1 inline" /> All lines full
              </Badge>
            )}
          </div>
          {/* Select production line to add machines to */}
          <p className="text-slate-400 text-xs">Select a production line to add machines:</p>
          <div className="flex flex-wrap gap-2">
            {(selectedFactory.productionLines ?? []).map((line: any) => {
              const lineMachines = machines.filter((m: any) => m.assignedLineId === line.id);
              const isSelected = selectedLineId === line.id;
              const isFull = lineMachines.length >= MAX_MACHINES_PER_LINE;
              return (
                <button
                  key={line.id}
                ...
                >
                  <div className="font-medium">{line.segment || "Idle"}</div>
                  <div className={`text-xs font-mono ${isFull ? 'text-orange-400' : 'text-emerald-400'}`}>
                    {lineMachines.length}/{MAX_MACHINES_PER_LINE} machines
                  </div>
                </button>
              );
            })}
                      ...
                      <div className="mt-2">
                        {pending ? (
                          ...
                          <Button
                            ...
                            disabled={!selectedLineId || factoryFull}
                            onClick={() => {
                              if (selectedLineId && cfg.baseCapacity > 0) {
                                toggleUpgradePurchase(`${cfg.type}__line:${selectedLineId}`);
                              } else {
                                toggleUpgradePurchase(cfg.type);
                              }
                            }}
                          >
                            <Plus className="w-3 h-3" />
                            {selectedLineId
                              ? `Add to ${(selectedFactory.productionLines ?? []).find((l: any) => l.id === selectedLineId)?.segment || "Line"}`
                              : "Select a line first"
                            }
                          </Button>
                        )}
      ...
      {/* Build New Factory lives in Upgrades tab (strategic investment) */}
    </motion.div>
  );
}
