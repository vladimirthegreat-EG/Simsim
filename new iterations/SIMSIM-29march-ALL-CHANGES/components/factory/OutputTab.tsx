  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="space-y-3">
      ...
      {segments.map((segment) => {
        ...
        // Use actual cost components from product, not hardcoded split
        const RAW_MATERIAL_COSTS: Record<string, number> = { Budget: 50, General: 100, Enthusiast: 200, Professional: 350, "Active Lifestyle": 150 };
        const matCost = RAW_MATERIAL_COSTS[segment] ?? Math.round(unitCost * 0.5);
        const laborCost = 20; // LABOR_COST_PER_UNIT from engine constants
        const ohCost = Math.max(0, unitCost - matCost - laborCost);
