...
export interface UIFactoryDecisions {
  efficiencyInvestment: {
    ...
    engineers: number;
    equipment: number;
    [key: string]: number; // Allow additional fields (supervisors, machinery, factory)
  };
...
export interface UIHRDecisions {
  ...
  /** Benefits package changes (health, retirement, PTO, etc.) */
  benefitChanges?: Record<string, number | boolean>;
}
...
export interface UIMarketingDecisions {
  ...
  productPricing?: Record<string, number>; // productId → newPrice
}
...
export interface UIRDDecisions {
  ...
  techUpgrades: string[];
  /** Product improvements — improve launched products' quality/features */
  productImprovements?: Array<{
    productId: string;
