...
export default function FactoryPage({ params }: PageProps) {
  ...
  const [lineWorkers, setLineWorkers] = useState<Record<string, number>>({});
  ...
  useEffect(() => { setFactoryDecisions({ maintenanceInvestment }); }, [maintenanceInvestment, setFactoryDecisions]);
  useEffect(() => {
    const esgObj = activeEsgInitiatives.reduce((acc: Record<string, boolean>, id: string) => { acc[id] = true; return acc; }, {});
    setFactoryDecisions({ esgInitiatives: esgObj });
  }, [activeEsgInitiatives, setFactoryDecisions]);
  useEffect(() => { setFactoryDecisions({ shiftMode }); }, [shiftMode, setFactoryDecisions]);
  ...
  const factoryPreview = useFactoryPreview(state, {
    ...
    maintenanceInvestment, shiftMode,
    productionLineDecisions, esgInitiatives: activeEsgInitiatives.reduce((acc: Record<string, boolean>, id: string) => { acc[id] = true; return acc; }, {}),
  } as any);
  ...
  const toggleUpgradePurchase = (upgradeId: string) => {
    // Handle machine sell requests (sell-{type})
    if (upgradeId.startsWith("sell-")) {
      const machineType = upgradeId.replace("sell-", "");
      setUpgradePurchases(prev => [...prev, { factoryId: selectedFactory.id, upgradeName: upgradeId }]);
      return;
    }

    // Handle line-targeted machine purchases (type__line:lineId)
    const lineMatch = upgradeId.match(/^(.+)__line:(.+)$/);
    if (lineMatch) {
      const [, machineType, lineId] = lineMatch;
      setUpgradePurchases(prev => [...prev, {
        factoryId: selectedFactory.id,
        upgradeName: machineType,
        targetLineId: lineId,
      } as any]);
      return;
    }
    ...
    const isMachineType = ["assembly_line", "injection_molder", "pcb_assembler", "robotic_arm", "conveyor_system", "packaging_system", "quality_scanner", "clean_room_unit", "cnc_machine", "welding_station", "paint_booth", "laser_cutter", "3d_printer", "clean_room", "testing_rig", "forklift_fleet"].includes(upgradeId);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-slate-800 border-slate-700 flex-wrap">
          ...
          {/* Output tab merged into Lines tab */}
        ...
        <TabsContent value="lines" className="space-y-6">
          <ProductionLinesTab
            ...
            pendingWorkerChanges={lineWorkers}
            pendingTargetChanges={lineTargets}
            onSetWorkers={(lineId, workers) => {
              setLineWorkers(prev => ({ ...prev, [lineId]: workers }));
              const prevStaffing = (productionLineDecisions as Record<string, unknown>)?.staffing as Array<{ lineId: string; workers: number; engineers: number; supervisors: number }> | undefined;
              setFactoryDecisions({
                productionLineDecisions: {
                  ...productionLineDecisions,
                  staffing: [
                    ...(prevStaffing?.filter(s => s.lineId !== lineId) || []),
