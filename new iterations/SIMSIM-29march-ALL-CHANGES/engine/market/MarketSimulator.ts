...
export class MarketSimulator {
  ...
  static simulate(
    ...
    for (const segment of CONSTANTS.SEGMENTS) {
      ...
      // Count ALL products in this segment (primary + secondary, across all teams) for crowding
      let totalProductsInSegment = 0;
      for (const team of teams) {
        totalProductsInSegment += team.state.products.filter(
          p => (p.segment === segment || p.secondarySegments?.includes(segment))
            && p.developmentStatus === "launched" && !p.discontinued
        ).length;
      }
      const crowdingFactor = calculateCrowdingFactor(totalProductsInSegment);
      crowdingStates.push({ segment, productCount: totalProductsInSegment, crowdingFactor });

      // Calculate scores for each product in this segment (multi-product per team)
      // Includes primary segment products + secondary segment products (at 60% effectiveness)
      const SECONDARY_SEGMENT_PENALTY = 0.60; // Products competing outside primary segment score 60%
      for (const team of teams) {
        const primaryProducts = team.state.products.filter(
          p => p.segment === segment && p.developmentStatus === "launched" && !p.discontinued
        );
        const secondaryProducts = team.state.products.filter(
          p => p.segment !== segment
            && p.developmentStatus === "launched"
            && !p.discontinued
            && p.secondarySegments?.includes(segment)
        );
        const teamProducts = [
          ...primaryProducts.map(p => ({ product: p, isSecondary: false })),
          ...secondaryProducts.map(p => ({ product: p, isSecondary: true })),
        ];

        if (teamProducts.length === 0) {
          // Team has no product in this segment — add empty position
          const position = this.calculateTeamPosition(team.id, team.state, null, segment, marketState);
          segmentPositions.push(position);
          positions.push(position);
          continue;
        }

        // Calculate cannibalization penalty for same-team products
        const cannibalizationPenalties: number[] = [];
        for (let pi = 0; pi < teamProducts.length; pi++) {
          let penalty = 1.0;
          if (teamProducts.length > 1) {
            for (let pj = 0; pj < teamProducts.length; pj++) {
              if (pi === pj) continue;
              const overlap = this.calculateFeatureOverlap(teamProducts[pi].product, teamProducts[pj].product);
              penalty *= 1 - (0.05 + overlap * 0.25);
            }
          }
          cannibalizationPenalties.push(penalty);
        }

        // Score each product independently
        for (let pi = 0; pi < teamProducts.length; pi++) {
          const { product, isSecondary } = teamProducts[pi];
          const position = this.calculateTeamPosition(team.id, team.state, product, segment, marketState);

          // Secondary segment penalty — products competing outside primary segment score 60%
          if (isSecondary && position.totalScore > 0) {
            position.totalScore *= SECONDARY_SEGMENT_PENALTY;
          }

          // Apply cannibalization penalty
          if (cannibalizationPenalties[pi] < 1.0 && position.totalScore > 0) {
            position.totalScore *= cannibalizationPenalties[pi];
          }
      ...
      if (totalProductsInSegment <= 1) {
        ...
        if (entrant && !alreadyHasBonus) {
          const underservedFactor = totalProductsInSegment === 0 ? 1.0 : 0.5;
      ...
      if (crowdingFactor < 1.0) {
        marketEvents.push({
          ...
          round: marketState.roundNumber,
          description: `${segment} segment is crowded (${totalProductsInSegment} products, ${((1 - crowdingFactor) * 100).toFixed(0)}% penalty)`,
          impact: "negative",
          segment,
        });
      }
      ...
      for (let i = 0; i < segmentPositions.length; i++) {
        ...
        // Per-product capacity: find the specific line for THIS product
        let productionCapacity = 0;
        if (team && position.product) {
          // Look for a production line assigned to this specific product
          for (const factory of team.state.factories) {
            const productLine = factory.productionLines?.find(
              l => l.productId === position.product!.id && l.status === "active"
            );
            if (productLine) {
              try {
                const analysis = getFactoryBottleneckAnalysis(team.state, factory.id);
                const lineResult = analysis[productLine.id] as { projectedOutput: number } | undefined;
                productionCapacity += lineResult?.projectedOutput ?? 0;
              } catch {
                // Fallback: use line target as rough capacity
                productionCapacity += productLine.targetOutput ?? 0;
              }
            }
          }
        } else if (team) {
          // No product — use segment-level capacity (for empty positions)
          productionCapacity = MarketSimulator.getTeamProductionCapacityFromLines(team.state, segment);
        }
      ...
      // v5.2 FIX: Only 30% of unfulfilled demand switches to competitors.
      // The rest is lost (customers wait, leave market, or don't buy).
      // This ensures market share advantages translate to real revenue differences
      // even when the leading team is capacity-constrained.
      // Supply Chain upgrade: stockoutReduction retains more demand for the team
      // Base: 30% of unmet demand switches to competitors, 70% lost
      // With stockoutReduction: the team keeps more of its own demand next round
      const DEMAND_SWITCH_RATE = 0.30;
      if (totalUnfulfilled > 0) {
        const switchableDemand = Math.floor(totalUnfulfilled * DEMAND_SWITCH_RATE);
        ...
        if (totalSpare > 0 && switchableDemand > 0) {
          // Supply Chain + Warehousing upgrades: teams with these upgrades capture more spillover
          const weightedSpare = teamsWithSpare.map(t => {
            const teamData = teams[segmentPositions[t.index]?.teamId ? teams.findIndex(tm => tm.id === segmentPositions[t.index].teamId) : t.index];
            const factory = teamData?.state?.factories?.[0];
            const stockoutBonus = 1 + ((factory as any)?.stockoutReduction ?? 0); // Supply Chain: +70% capture
            const spikeBonus = 1 + ((factory as any)?.demandSpikeCapture ?? 0) * 0.5; // Warehousing: +45% capture
            return { ...t, weight: t.spareCapacity * stockoutBonus * spikeBonus };
          });
          const totalWeight = weightedSpare.reduce((s, t) => s + t.weight, 0);
          for (const t of weightedSpare) {
            const extraUnits = Math.min(
              Math.floor(switchableDemand * (t.weight / totalWeight)),
              t.spareCapacity
            );
  ...
  /**
   * Calculate feature overlap between two products (cosine similarity of feature emphasis).
   * Returns 0.0 (completely different) to 1.0 (identical features).
   * Used for cannibalization: higher overlap = more cannibalization.
   */
  static calculateFeatureOverlap(productA: any, productB: any): number {
    const featuresA = productA.featureEmphasis || productA.features || {};
    const featuresB = productB.featureEmphasis || productB.features || {};
    const allKeys = new Set([...Object.keys(featuresA), ...Object.keys(featuresB)]);
    if (allKeys.size === 0) return 0.5; // No feature data — assume moderate overlap

    let dotProduct = 0;
    let magA = 0;
    let magB = 0;
    for (const key of allKeys) {
      const a = featuresA[key] ?? 0;
      const b = featuresB[key] ?? 0;
      dotProduct += a * b;
      magA += a * a;
      magB += b * b;
    }
  ...
  static calculateTeamPosition(
    ...
    // Product aging — older products lose competitive edge (Capsim pattern)
    // 4-round grace period (no aging), then 5% per round, floor at 40%
    // Starter products survive until R4-5, then need first R&D revision
    const productAge = product.launchRound != null
      ? Math.max(0, (marketState.roundNumber ?? 1) - product.launchRound)
      : 0;
    const agingAfterGrace = Math.max(0, productAge - 4); // 4-round grace period
    const agingPenalty = Math.max(0.40, 1 - agingAfterGrace * 0.05);
    qualityScore *= agingPenalty;
    featureScore *= agingPenalty;
    ...
    // v5.1: Reduced from 1.08/1.04 to 1.05/1.02 to curb premium strategy dominance
    const gradeMultiplier = product.qualityGrade === "artisan" ? 1.05
      : product.qualityGrade === "premium" ? 1.02
      : 1.0;
    ...
    // Customer loyalty bonus — consistent supply builds loyalty
    const teamLoyalty = state.customerLoyalty?.[segment] ?? 50;
    const loyaltyBonus = (teamLoyalty / 100) * 0.05; // 0% to 5% score boost
    totalScore *= (1 + loyaltyBonus);
   ...
   */
  // FLAG: CQ-02 — These inline expectations should move to CONSTANTS.SEGMENT_QUALITY_EXPECTATIONS (P2 post-launch)
   ...
   * @deprecated LEGACY — kept only for test comparison. Production code uses getTeamProductionCapacityFromLines().
   ...
   * Line-based capacity — no legacy fallback. 0 capacity if no lines for segment.
   */
  /**
   * Line-based production capacity (primary system).
   * No legacy fallback — if no lines configured for a segment, capacity = 0.
   * Teams must set up production lines + assign machines to produce.
   */
  static getTeamProductionCapacityFromLines(
    state: TeamState,
    segment: Segment,
    _allocationPercent?: number // kept for API compat, not used in line system
  ): number {
    let totalCapacity = 0;
    for (const factory of state.factories) {
      try {
        ...
        // Bottleneck analysis failed for this factory — skip it (0 capacity)
      }
    }
  ...
  static calculateMarketShares(positions: TeamMarketPosition[]): number[] {
    ...
    const MAX_SHARE = 0.80;  // Capsim pattern: even monopoly can't capture 100% (some customers don't buy)
    const teamsWithProduct = shares.filter(s => s > 0).length;
    if (teamsWithProduct > 0) {
      shares = shares.map(s => {
        if (s <= 0) return s;
        return Math.min(MAX_SHARE, Math.max(MIN_SHARE, s));
      });
   ...
   */
  // FLAG: CQ-03 — These inline elasticities should move to CONSTANTS.SEGMENT_PRICE_ELASTICITIES (P2 post-launch)
  ...
  static updateCustomerLoyalty(
    ...
    const loyalty: Record<Segment, number> = currentLoyalty
      ? { ...currentLoyalty }
      : { "Budget": 50, "General": 50, "Enthusiast": 50, "Professional": 50, "Active Lifestyle": 50 }; // FLAG: CQ-04 — move to CONSTANTS.DEFAULT_LOYALTY_VALUES (P2)
    ...
    for (const segment of CONSTANTS.SEGMENTS) {
      ...
      const share = marketShareBySegment?.[segment] ?? 0;
      const current = loyalty[segment] ?? 50;

      if (share > 0.05) {
        // Present in market: loyalty drifts toward 50 + (share * 100), capped at 100
        const target = Math.min(100, 50 + share * 100);
        const shareGain = (target - current) * 0.2;
        // Also factor in fulfillment-based gain
        const fulfillmentGain = sold > 0 ? 2 + fulfillmentRate * 3 : 0;
        loyalty[segment] = Math.min(100, Math.round(current + shareGain + fulfillmentGain));
      } else if (sold > 0) {
        // Low share but still selling: small loyalty gain
        const loyaltyGain = 2 + fulfillmentRate * 3;
        loyalty[segment] = Math.min(100, loyalty[segment] + loyaltyGain);
      } else {
        // Absent from segment: loyalty decays toward 30
        loyalty[segment] = Math.round(current + (30 - current) * 0.15);
      }
    }
    ...
    for (const segment of CONSTANTS.SEGMENTS) {
      ...
      for (const team of teams) {
        for (const product of team.state.products) {
          if (product.segment === segment && product.developmentStatus === "launched" && !product.discontinued) {
