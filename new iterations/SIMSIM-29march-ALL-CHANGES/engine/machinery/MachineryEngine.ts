...
export class MachineryEngine {
  ...
  static process(
    ...
    for (const machine of machines) {
      ...
      let breakdownChance = this.calculateBreakdownChance(machine);
      // Apply factory-level breakdown reduction from upgrades (IoT Integration: -15%)
      if ((machine as any)._factoryBreakdownReduction) {
        breakdownChance *= (1 - (machine as any)._factoryBreakdownReduction);
      }
