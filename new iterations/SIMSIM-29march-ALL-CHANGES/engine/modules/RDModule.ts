...
export class RDModule {
  ...
  private static processInternal(
    ...
    if (decisions.newProducts) {
      // Enforce product cap (Capsim pattern: forces strategic choices)
      const activeProducts = newState.products.filter(p => !p.discontinued).length;
      const maxProducts = CONSTANTS.MAX_PRODUCTS_PER_TEAM ?? 10;

      for (const productSpec of decisions.newProducts) {
        const currentActive = newState.products.filter(p => !p.discontinued).length;
        if (currentActive >= maxProducts) {
          messages.push(`Product cap reached (${maxProducts}). Discontinue a product before launching new ones.`);
          break;
        }
    ...
    for (const product of newState.products) {
      if (product.developmentStatus === "ready") {
        ...
        product.launchRound = ctx?.roundNumber ?? 0;
      ...
      for (const improvement of decisions.productImprovements) {
        ...
        if (product) {
          ...
          if (newState.cash >= improvementCost && newState.rdProgress >= rdPointsRequired) {
            ...
            // Capsim pattern: R&D revision resets the aging clock
            // Product is "refreshed" — competitive positioning is updated
            product.launchRound = ctx?.roundNumber ?? newState.round ?? product.launchRound;

            // Recalculate featureSet for archetype products (incorporates newly unlocked techs)
            if (product.featureSet && product.archetypeId) {
              const archetype = getArchetype(product.archetypeId);
              if (archetype) {
                const currentUnlockedTechs = newState.unlockedTechnologies || [];
                const baseFeatures = RDExpansions.calculateProductFeatureSet(currentUnlockedTechs, newState.researchDecayTimers);
                for (const family of TECH_FAMILIES) {
                  const emphasis = archetype.featureEmphasis[family] ?? 1.0;
                  product.featureSet[family] = Math.min(100, Math.round(baseFeatures[family] * emphasis));
                }
                product.appliedTechs = [...currentUnlockedTechs];
              }
            }

            // Recalculate unit cost after quality change
            const seg = product.segment as Segment;
            const matCost = CONSTANTS.RAW_MATERIAL_COST_PER_UNIT[seg];
            const labCost = CONSTANTS.LABOR_COST_PER_UNIT;
            const ovrCost = CONSTANTS.OVERHEAD_COST_PER_UNIT;
            const qPremium = (product.quality - 50) * 0.5;
            product.unitCost = matCost + labCost + ovrCost + qPremium;

            newState.rdProgress -= rdPointsRequired;
            totalCosts += improvementCost;
            messages.push(`Revised ${product.name}: Q+${improvement.qualityIncrease || 0}, F+${improvement.featuresIncrease || 0} (aging reset)`);
          } else {
    ...
    // Process product discontinuations
    if (decisions.discontinueProducts) {
      for (const productId of decisions.discontinueProducts) {
        ...
        if (product && !product.discontinued) {
          ...
          messages.push(`${product.name} has been discontinued`);
        }
      }
    }

    // Product aging is handled by MarketSimulator (launchRound-based scoring penalty)
    // R&D revisions reset launchRound, which resets the aging clock
    // No direct stat decay here — keeps product.quality/features stable for display
    // Players see aging impact through market share loss, not raw stat drops

    // Patents are now managed exclusively through the PatentEngine (file/license/challenge)
    // Legacy number-based patents are preserved for backward compatibility but no new ones auto-generate
    ...
    // Minimal bridge: legacy process_optimization grants Tier 1 starter nodes
    // Players must research higher tiers through the expansion tech tree
    if (newState.unlockedTechnologies.includes("process_optimization")) {
      const starterNodes = ["bat_1", "cam_1", "ai_1", "dur_1", "dsp_1", "con_1"];
      for (const nodeId of starterNodes) {
        if (!newState.unlockedTechnologies.includes(nodeId)) {
          newState.unlockedTechnologies.push(nodeId);
        }
      }
    }
    ...
    // R&D maintenance cost: $1M per launched product per round (Capsim pattern)
    // Represents ongoing quality control, compliance, certification, and support engineering
    // Forces teams to consider cost of maintaining large product portfolios
    const launchedProducts = newState.products.filter(p => p.developmentStatus === "launched" && !p.discontinued);
    const rdMaintenanceCost = launchedProducts.length * 1_000_000;
    totalCosts += rdMaintenanceCost;
    if (launchedProducts.length > 0) {
      messages.push(`R&D maintenance: $${(rdMaintenanceCost / 1e6).toFixed(1)}M for ${launchedProducts.length} active products`);
    }
  ...
  static createArchetypeProduct(
    ...
    const baseFeatures = RDExpansions.calculateProductFeatureSet(unlockedTechs, undefined);
    ...
    return {
      ...
      secondarySegments: archetype.secondarySegments.length > 0 ? [...archetype.secondarySegments] : undefined,
