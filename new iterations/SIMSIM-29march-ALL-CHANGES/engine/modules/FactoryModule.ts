...
export class FactoryModule {
  ...
  private static processInternal(
    ...
    // Process new factory construction (1-round delay — Capsim/BSG pattern)
    if (decisions.newFactories) {
      for (const nf of decisions.newFactories) {
        ...
        if (newState.cash >= cost) {
          ...
          // Factory starts under construction — operational next round
          (newFactory as unknown as Record<string, unknown>).underConstruction = true;
          (newFactory as unknown as Record<string, unknown>).constructionRoundsRemaining = 1;
          newFactory.efficiency = 0; // Can't produce while under construction
          newState.factories.push(newFactory);
          totalCosts += cost;
          messages.push(`New ${tierConfig.label} factory "${nf.name}" under construction in ${nf.region} — operational next round ($${(cost / 1_000_000).toFixed(0)}M)`);
        } else {
    ...
    // Advance construction countdowns for factories under construction
    for (const factory of newState.factories) {
      const fc = factory as unknown as Record<string, unknown>;
      if (fc.underConstruction && typeof fc.constructionRoundsRemaining === "number") {
        fc.constructionRoundsRemaining = (fc.constructionRoundsRemaining as number) - 1;
        if ((fc.constructionRoundsRemaining as number) <= 0) {
          fc.underConstruction = false;
          factory.efficiency = 0.7; // Standard starting efficiency
          messages.push(`Factory "${factory.name}" construction complete — now operational!`);
        }
      }
    }

    // Continuous improvement: +1% efficiency per round if upgrade is enabled (caps at +10%)
    for (const factory of newState.factories) {
      const fc = factory as unknown as Record<string, unknown>;
      if (fc.continuousImprovementEnabled) {
        const currentBonus = (fc.continuousImprovementBonus as number) ?? 0;
        if (currentBonus < 0.10) {
          fc.continuousImprovementBonus = Math.min(0.10, currentBonus + 0.01);
          factory.efficiency = Math.min(1.0, factory.efficiency + 0.01);
          messages.push(`Factory "${factory.name}" continuous improvement: +1% efficiency (total bonus: +${((fc.continuousImprovementBonus as number) * 100).toFixed(0)}%)`);
        }
      }
    }
    ...
    if (decisions.machineryDecisions && ctx) {
      ...
      // Collect all existing machines across factories + stamp factory breakdown reduction
      const allMachines: import("../machinery/types").Machine[] = [];
      for (const [factoryId, fms] of Object.entries(newState.machineryStates)) {
        const factory = newState.factories.find(f => f.id === factoryId);
        const bdr = (factory as any)?.breakdownReduction ?? 0;
        for (const m of fms.machines) {
          if (bdr > 0) (m as any)._factoryBreakdownReduction = bdr;
          allMachines.push(m);
        }
      }
      ...
      for (const [factoryId, fms] of Object.entries(newState.machineryStates)) {
        ...
        if (factory && fms.machines.length > 0) {
          ...
          factory.defectRate = Math.max(0.005, (factory.defectRate ?? 0) - defectReduction); // Min 0.5% defects — real manufacturing never hits zero
        }
      }
  ...
  static calculateProduction(
    ...
    // Cap total automation multiplier at 1.50 (50% max production boost from upgrades)
    automationMultiplier = Math.min(1.50, automationMultiplier);
    ...
    // FIX #9: Worker accuracy reduces defect rate
    const workerAccuracy = (factory as any).avgWorkerAccuracy ?? 75;
    const accuracyModifier = 1.3 - (workerAccuracy / 100) * 0.5; // acc=100: 0.8, acc=75: 1.05, acc=0: 1.3
    // Quality Lab: qaSpeedBonus reduces defects by catching more issues (30% bonus = 15% fewer defects)
    const qaBonus = 1 - ((factory as any).qaSpeedBonus ?? 0) * 0.5; // 0.30 * 0.5 = 0.15 → 15% defect reduction
    const defectRate = factory.defectRate * accuracyModifier * qaBonus;
  ...
  static calculateRecommendedStaffing(factory: Factory): {
    ...
    const machines = factory.productionLines.reduce((sum, line) => sum + Math.ceil((line.capacity ?? 0) / 1000), 0) || 10;
  ...
  static calculateWasteMetrics(
    ...
    for (const factory of factories) {
      ...
      const upgradeReduction = (factory.wasteCostReduction ?? 0) * baseWasteRate; // Waste to Energy: 20% of base waste rate
