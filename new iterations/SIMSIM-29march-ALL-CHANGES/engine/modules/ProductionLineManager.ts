...
export function calculateLineOutput(
  ...
  shiftMode: "single" | "double" | "overtime" = "single"
): BottleneckResult {
  ...
  // ── Shift mode multiplier (Capsim pattern) ──
  // Single: 1x output, base labor cost
  // Double: 2x output, 1.7x labor cost (second shift is less efficient)
  // Overtime: 1.5x output, 1.6x labor cost (fatigue reduces efficiency)
  const shiftMultiplier = shiftMode === "double" ? 2.0 : shiftMode === "overtime" ? 1.5 : 1.0;

  // ── Final output ──
  let projectedOutput = rawCapacity * staffingRatio * efficiency * sharedScale * qualityYield * shiftMultiplier;
