...
export class IncomeStatementEngine {
  ...
  static generate(state: TeamState, round: number): IncomeStatement {
    ...
    // NaN guard: ensure all financial outputs are finite numbers
    ...
    return {
      ...
      grossMargin: revenue.total > 0 ? safe((grossProfit / revenue.total) * 100) : 0,
      operatingExpenses: opex,
      operatingIncome: safe(operatingIncome),
      operatingMargin: revenue.total > 0 ? safe((operatingIncome / revenue.total) * 100) : 0,
      interestExpense: safe(interestExpense),
      interestIncome: safe(interestIncome),
      otherIncome: 0,
      otherExpenses: 0,
      incomeBeforeTax: safe(incomeBeforeTax),
      taxExpense: safe(taxExpense),
      effectiveTaxRate:
        incomeBeforeTax > 0 ? safe((taxExpense / incomeBeforeTax) * 100) : 0,
      netIncome: safe(netIncome),
      netMargin: revenue.total > 0 ? safe((netIncome / revenue.total) * 100) : 0,
      eps: this.calculateEPS(state, safe(netIncome)),
    };
  }
  ...
  private static calculateCOGS(state: TeamState): IncomeStatement["cogs"] {
    ...
    // FIX #8: Use actual employee salaries instead of hardcoded $45K
    const workers = Array.isArray(state.employees)
      ? state.employees.filter((e: any) => e.role === "worker")
      : [];
    const workerCount = workers.length || ((state.employees as any)?.workers ?? 0);
    const actualWorkerSalaries = workers.length > 0
      ? workers.reduce((sum: number, e: any) => sum + (e.salary || 45000), 0)
      : workerCount * 45000;
    const directLabor = (actualWorkerSalaries / 12) * laborMult;
  ...
  private static calculateOperatingExpenses(
    ...
    // employees may be an array of Employee objects or an object with counts
    const opWorkerCount = Array.isArray(state.employees)
      ? state.employees.filter((e: any) => e.role === "worker").length
      : (state.employees?.workers ?? 0);
    const engineerCount = Array.isArray(state.employees)
      ? state.employees.filter((e: any) => e.role === "engineer").length
      : (state.employees?.engineers ?? 0);
    const supervisorCount = Array.isArray(state.employees)
      ? state.employees.filter((e: any) => e.role === "supervisor").length
      : (state.employees?.supervisors ?? 0);
    const engineerSalaries = engineerCount * (85000 / 12);
    const supervisorSalaries = supervisorCount * (75000 / 12);
    const overheadStaff = Math.floor(opWorkerCount * 0.1); // 10% overhead
