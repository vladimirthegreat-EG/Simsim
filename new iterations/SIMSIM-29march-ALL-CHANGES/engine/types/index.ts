...
export const CONSTANTS = {
  ...
  ESG_INITIATIVES: {
    ...
    executivePayRatio: { cost: 500_000, points: 100 },  // F10 FIX: was 0, now has admin cost
  },
  ...
  // Training
  TRAINING_FATIGUE_THRESHOLD: 2,        // FIX #6: Programs per year before diminishing returns (was 1)
  TRAINING_FATIGUE_PENALTY: 0.15,       // FIX #6: 15% effectiveness reduction per extra program (was 30%)
  ...
  RAW_MATERIAL_COST_PER_UNIT: {
    "Budget": 90,              // $200 price → 55% margin target → $110 cost needed → $90 mat
    "General": 200,            // $450 price → 38% margin → $280 cost → $200 mat
    "Enthusiast": 320,         // $700 price → 40% margin → $420 cost → $320 mat
    "Professional": 550,       // $1200 price → 45% margin → $660 cost → $550 mat
    "Active Lifestyle": 250,   // $550 price → 38% margin → $340 cost → $250 mat
  } as Record<Segment, number>,
  LABOR_COST_PER_UNIT: 45,              // Base labor cost per unit (Capsim: 25-35% of COGS)
  OVERHEAD_COST_PER_UNIT: 35,           // Overhead per unit (facility, utilities, quality, admin)
  ...
  RB_MAX_COST_RELIEF: 0.10,             // F7 FIX: 10% (was 18%)
  ...
  BRAND_LOW_MULTIPLIER: 0.70,           // -30% brand score for weak brands
  BRAND_HIGH_MULTIPLIER: 1.15,          // +15% brand score for strong brands (makes Premium strategy viable)
  ...
  SOFTMAX_TEMPERATURE: 3.5,             // Higher temp = more distributed shares (Capsim-like 20-30% ranges, not winner-take-all)
  ...
  QUALITY_FEATURE_BONUS_CAP: 1.50,      // v5.2: raised from 1.15 to allow quality differentiation in low-expectation segments (Budget q=60 vs q=90 were both capped at 1.15)
  QUALITY_MARKET_SHARE_BONUS: 0.0005,   // +0.05% market share per quality point (v5.1: reduced from 0.0011 to curb premium dominance — was 56.8%, target ≤45%)
  ...
  SEGMENT_WEIGHTS: {
    // v5.1: Budget price 40→45, Professional quality 30→27 — cost-cutter was 8.4% (target ≥12%), premium was 56.8% (target ≤45%)
    "Budget":           { price: 45, quality: 20, brand: 10, esg: 7,  features: 18 },
    "General":          { price: 30, quality: 22, brand: 16, esg: 10, features: 22 },
    "Enthusiast":       { price: 12, quality: 30, brand: 8,  esg: 5,  features: 45 },
    "Professional":     { price: 25, quality: 27, brand: 10, esg: 17, features: 21 },
    "Active Lifestyle": { price: 22, quality: 32, brand: 10, esg: 10, features: 26 },
  ...
  MAX_PRODUCTS_PER_TEAM: 10,              // Cap: 5 segments × 2 archetypes each (forces strategic choice)
  MAX_FACTORIES_PER_TEAM: 3,             // Cap: 1 starter + 2 expansions (18 lines total)
  ...
  DEFAULT_PRODUCT_CAPACITY: 200_000,      // v5.2: Raised from 50K to ensure teams start with excess capacity (Capsim pattern: ~1.2x demand). At 50K, all teams hit capacity ceiling and market share has no effect on revenue.
