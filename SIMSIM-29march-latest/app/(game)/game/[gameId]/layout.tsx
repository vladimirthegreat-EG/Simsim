...
export default function GamePageLayout({ children, params }: LayoutProps) {
  ...
  // Extract CURRENT round cash from team state (not game starting cash)
  // This is the actual cash available for spending decisions this round
  const startingCash = useMemo(() => {
    if (!teamState?.state) return 175_000_000; // fallback matches DEFAULT_STARTING_CASH
    try {
      const parsed = typeof teamState.state === "string"
        ? JSON.parse(teamState.state)
        : teamState.state;
      return parsed.cash ?? 175_000_000;
    } catch {
      return 175_000_000;
    }
  }, [teamState?.state]);
