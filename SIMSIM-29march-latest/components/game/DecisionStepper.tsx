...
const STEPS: StepDef[] = [
  ...
  { key: "SC" as GameModule, label: "Source", shortLabel: "Supply", narrative: "Source", icon: Package, path: "/supply-chain", color: "text-teal-400" },
...
// The expected submission order (SC excluded — orders placed directly, no submit flow)
...
export function DecisionStepper({ gameId, startingCash }: DecisionStepperProps) {
  ...
  // Progress bar: count enabled modules and how many are submitted
  const enabledModules = STEP_ORDER.filter((k) => isModuleEnabled(MODULE_TO_COMPLEXITY[k]));
  const submittedCount = enabledModules.filter((k) => submissionStatus[k].isSubmitted).length;
  const totalEnabled = enabledModules.length;
  const progressPercent = totalEnabled > 0 ? (submittedCount / totalEnabled) * 100 : 0;
  ...
  return (
    <div className="border-b border-slate-700/50 bg-slate-800/40">
      ...
      <div className="flex items-center justify-center gap-1 px-4 py-2 overflow-x-auto">
        {STEPS.map((step, i) => {
          const status = submissionStatus[step.key] ?? { isSubmitted: false, isDirty: false, isLocked: false, isSubmitting: false, lastSubmittedAt: null, error: null };
          ...
          // Is this the NEXT module to submit (first unsubmitted in order)?
          const isNextToSubmit = step.key === firstUnsubmitted;
            ...
            <div key={step.key} className="flex items-center">
              <Link
                ...
                className={cn(
                  ...
                  isNextToSubmit && !isActive && "ring-2 ring-emerald-400/60 animate-pulse-ring"
                )}
              >
                {/* Pulsing ring overlay for "start here" indicator */}
                {isNextToSubmit && !isActive && (
                  <span className="absolute inset-0 rounded-md ring-2 ring-emerald-400 animate-ping opacity-30 pointer-events-none" />
                )}
                ...
                {/* Cost chip — show when submitted OR dirty (has unsaved changes) */}
                {(status?.isSubmitted || status?.isDirty) && cost !== 0 && (
                  <span className={cn(
      ...
      <div className="px-4 pb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-slate-400 tabular-nums whitespace-nowrap">
            {submittedCount} of {totalEnabled} submitted
          </span>
        </div>
      </div>
      ...
      {/* H2 FIX: "Confirm All & Lock" button — appears when all modules submitted */}
      <ConfirmAllButton />
    </div>
  );
}
...
/**
 * H2 FIX: Button to lock all decisions for the round.
 * Only appears when all enabled modules are submitted.
 * Calls decision.lockAll which prevents further changes.
 */
function ConfirmAllButton() {
  const allSubmitted = useAllModulesSubmitted();
  const submissionStatus = useDecisionStore((s) => s.submissionStatus);
  const setLocked = useDecisionStore((s) => s.setLocked);
  const [isLocking, setIsLocking] = useState(false);
  const [isLocked, setIsLockedState] = useState(false);

  const lockAll = trpc.decision.lockAll.useMutation({
    onSuccess: (result) => {
      if (result.success) {
        // Update store to show locked state
        const modules: GameModule[] = ["RD", "FACTORY", "HR", "FINANCE", "MARKETING"];
        modules.forEach(m => setLocked(m, true));
  ...
  const anyLocked = Object.values(submissionStatus).some(s => s.isLocked);
  if (anyLocked || isLocked) {
    return (
      <div className="flex items-center justify-center gap-2 px-4 py-2 bg-emerald-500/10 border-t border-emerald-500/20">
        <Lock className="w-4 h-4 text-emerald-400" />
        <span className="text-emerald-300 text-sm font-medium">All decisions locked for this round</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center justify-center gap-3 px-4 py-2.5 bg-emerald-500/5 border-t border-emerald-500/20">
      <span className="text-emerald-300 text-sm">All modules submitted!</span>
      <button
        onClick={() => {
          setIsLocking(true);
          lockAll.mutate();
      ...
      >
        ...
        {isLocking ? "Locking..." : "Confirm All & Lock Round"}
      </button>
    </div>
  );
}
