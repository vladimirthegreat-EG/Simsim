...
export class BalanceSheetEngine {
  ...
  private static calculatePPE(
    ...
    const productionLinesAtCost = state.factories.reduce(
      (sum, factory) => sum + (Array.isArray(factory.productionLines) ? factory.productionLines.length : 0) * 5_000_000, // $5M per line
      0
    );
