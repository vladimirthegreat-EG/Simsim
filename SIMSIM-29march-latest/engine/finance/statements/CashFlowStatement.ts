...
export class CashFlowEngine {
  ...
  static generate(
    ...
    // NaN guard for top-level cash flow values
    const safeCF = (v: number): number => isFinite(v) ? v : 0;

    // Net cash change
    const netCashChange =
      safeCF(operating.netCashFromOperations) +
      safeCF(investing.netCashFromInvesting) +
      safeCF(financing.netCashFromFinancing);

    // Beginning and ending cash
    const beginningCash = previousBalance?.assets.currentAssets.cash ?? (safeCF(state.cash) - netCashChange);
    const endingCash = safeCF(state.cash);

    // Free cash flow
    const freeCashFlow =
      safeCF(operating.netCashFromOperations) + safeCF(investing.capitalExpenditures);
  ...
  private static calculateOperatingCashFlow(
    ...
    // NaN guard: ensure all cash flow inputs are finite
    ...
    const netCashFromOperations =
      ...
      safe(amortization) -
      safe(changeInAR) -
      safe(changeInInventory) +
      safe(changeInAP) +
      safe(changeInAccruedExpenses);

    return {
      netIncome: safe(netIncome),
      depreciation: safe(depreciation),
      amortization: safe(amortization),
      changeInAR: safe(changeInAR),
      changeInInventory: safe(changeInInventory),
      changeInAP: safe(changeInAP),
      changeInAccruedExpenses: safe(changeInAccruedExpenses),
