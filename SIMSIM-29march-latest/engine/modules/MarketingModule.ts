...
export class MarketingModule {
  ...
  private static processInternal(
    ...
    // ESG → Brand link: high ESG score provides a brand value boost
    // ESG > 500 → small boost, ESG > 700 → larger boost
    // This creates the "ESG → brand → revenue" pedagogical pathway
    const esgScore = newState.esgScore ?? 0;
    if (esgScore > 300) {
      const esgBrandBoost = ((esgScore - 300) / 700) * 0.02; // 0 to 0.02 per round
      newState.brandValue = Math.min(1, newState.brandValue + esgBrandBoost);
    }
